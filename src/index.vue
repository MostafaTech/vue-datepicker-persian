...
import { isValidDate, isValidRange, isDateObejct, isPersianDateObejct, isPlainObject, formatDate, parseDate } from '@/utils/index'
...
export default {
  ...
  computed: {
    language () {
      if (isPlainObject(this.lang)) {
        return { ...Languages.fa, ...this.lang }
      }
      return Languages[this.lang] || Languages.fa
    },
    innerPlaceholder () {
      ...
      const arr = [
        {
          text: pickers[0],
          onClick (self) {
            self.currentValue = [ new PersianDate(), new PersianDate(Date.now() + 3600 * 1000 * 24 * 7) ]
        ...
        },
        {
          text: pickers[1],
          onClick (self) {
            self.currentValue = [ new PersianDate(), new PersianDate(Date.now() + 3600 * 1000 * 24 * 30) ]
            self.updateDate(true)
          }
        },
        {
          text: pickers[2],
          onClick (self) {
            self.currentValue = [ new PersianDate(Date.now() - 3600 * 1000 * 24 * 7), new PersianDate() ]
            self.updateDate(true)
          }
        },
        {
          text: pickers[3],
          onClick (self) {
            self.currentValue = [ new PersianDate(Date.now() - 3600 * 1000 * 24 * 30), new PersianDate() ]
            self.updateDate(true)
          }
        }
      ]
  ...
  methods: {
    ...
    dateEqual (a, b) {
      return isPersianDateObejct(a) && isPersianDateObejct(b) && a.getTime() === b.getTime()
    },
    rangeEqual (a, b) {
    ...
    selectRange (range) {
      ...
      this.currentValue = [ new PersianDate(range.start), new PersianDate(range.end) ]
    ...
    handleValueChange (value) {
      if (!this.range) {
        this.currentValue = isValidDate(value) ? new PersianDate(value) : null
      } else {
        this.currentValue = isValidRange(value) ? [new PersianDate(value[0]), new PersianDate(value[1])] : [null, null]
      }
    },
    selectDate (date) {
      console.log('index.selectDate', date)
      this.currentValue = date
