...
export default {
  ...
  computed: {
    currentHours () {
      return this.value ? new PersianDate(this.value).getHours() : 0
    },
    currentMinutes () {
      return this.value ? new PersianDate(this.value).getMinutes() : 0
    },
    currentSeconds () {
      return this.value ? new PersianDate(this.value).getSeconds() : 0
    }
  },
  methods: {
    stringifyText (value) {
    ...
    selectTime (time) {
      ...
      this.$emit('select', new PersianDate(time))
    },
    pickTime (time) {
      if (typeof this.disabledTime === 'function' && this.disabledTime(time)) {
        return
      }
      this.$emit('pick', new PersianDate(time))
    },
  ...
  render (h) {
    const date = new PersianDate(this.value)
    ...
    if (Array.isArray(pickers) && pickers.length) {
      pickers = pickers.map(picker => {
        ...
        const time = new PersianDate(date).setHours(pickHours, pickMinutes, 0)
        return (
          <li
            class={{
    ...
    const hours = Array.apply(null, { length: 24 }).map((_, i) => {
      const time = new PersianDate(date).setHours(i)
      return <li
        class={{
          'cell': true,
    ...
    const minutes = Array.apply(null, { length }).map((_, i) => {
      const value = i * step
      const time = new PersianDate(date).setMinutes(value)
      return <li
        class={{
          'cell': true,
    ...
    const seconds = Array.apply(null, { length: 60 }).map((_, i) => {
      const time = new PersianDate(date).setSeconds(i)
      return <li
        class={{
          'cell': true,
